...
class LoanController extends Controller
{
    ...
    public function update(UpdateLoan $request, $id)
    {
        $loan = Loan::find($id);
        $loan->loan_type = $request->input('loan_type');
        $loan->loan_no = $request->input('loan_no');
    ...
    public function paid_loan(Request $request)
    {
        $loan_id = $request->input('loan_id');
        $Loan = new Loan;
        $Loan = Loan::find($loan_id);
        $Loan->paidup_amount = $request->input('paidup_amount');
        $Loan->paidup_date = $request->input('paidup_date');
        $Loan->status = 0;
        $Loan->update();
        return redirect('/loan-paid')->with('status', 'Paidup Loan Successfully');
    }
}
